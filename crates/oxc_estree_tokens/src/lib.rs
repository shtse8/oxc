...
pub struct EstreeTokenOptions {
    ...
    pub use_ast_identifier_spans: bool,
    pub exclude_legacy_keyword_identifiers: bool,
    pub decode_identifier_escapes: bool,
    pub treat_legacy_identifier_names_as_keywords: bool,
}
...
impl EstreeTokenOptions {
    pub const fn test262() -> Self {
        Self {
            ...
            use_ast_identifier_spans: true,
            exclude_legacy_keyword_identifiers: true,
            decode_identifier_escapes: true,
            treat_legacy_identifier_names_as_keywords: false,
        }
    }
    ...
    pub const fn linter() -> Self {
        Self {
            ...
            exclude_legacy_keyword_identifiers: true,
            decode_identifier_escapes: false,
            treat_legacy_identifier_names_as_keywords: true,
        }
    }
}
...
pub fn to_estree_tokens<'a>(
    ...
    for token in tokens {
        ...
        let is_ast_identifier =
            options.use_ast_identifier_spans && context.ast_identifier_spans.contains(&span_utf16);
        ...
        let treat_legacy_identifier_as_keyword = options.treat_legacy_identifier_names_as_keywords
            && kind == Kind::Ident
            && matches!(source_value, "let" | "static" | "yield");
        let token_type = if treat_legacy_identifier_as_keyword {
            "Keyword"
        } else {
            token_type(
                kind,
